...
driver.get("http://cgatnew.gov.in/catweb/madrasnew/services/judgement_status.php")
...
def get_data(from_date, from_month, from_year, to_date, to_month, to_year):
	...
	for i in range(1,row_count+3):
		...
		try:
			x = (driver.find_element_by_xpath('/html/body/table[6]/tbody/tr[' + str(i) + ']/td')).text
			if (x == 'FINAL ORDER'):
				fin_order_index = i
			elif (x == 'ORAL ORDER'):
				or_order_index = i
	...
	if(oo == 0):
		or_order_index = row_count+2
	
	for i in range(1,row_count+3):

		
			if (i == fin_order_index):
				sr_no.append('FINAL ORDER')
				...
				r = (driver.find_element_by_xpath('/html/body/table[6]/tbody/tr[' + str(i) + ']/td[1]')).text
				sr_no.append(r)
